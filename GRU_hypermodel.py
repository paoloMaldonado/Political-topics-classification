...
class BiGRUHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        x_prev = layers.Bidirectional(layers.GRU(units=hp_units, name="prev_gru"), name="prev_bidirectional")(x_prev)
        mixed_layers.append(x_prev)

        # for current phrase
        x_curr = layers.Masking(mask_value=0.)(phrase_in)
        x_curr = layers.Bidirectional(layers.GRU(units=hp_units, name="curr_gru"), name="curr_bidirectional")(x_curr)
...
class GRUHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        # for previous phrase
        x_prev = layers.Masking(mask_value=0.)(prev_phrase_in)
        x_prev = layers.GRU(units=hp_units, name="prev_gru")(x_prev)
        mixed_layers.append(x_prev)

        # for current phrase
        x_curr = layers.Masking(mask_value=0.)(phrase_in)
        x_curr = layers.GRU(units=hp_units, name="curr_gru")(x_curr)
        mixed_layers.append(x_curr)
